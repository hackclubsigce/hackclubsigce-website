...
const Testimonials = () => {
  ...
  return (
    <Box as="section" id="sessions" sx={styles.testimonials}>
      <Container>
        <BlockTitle
          slogan="Sessions"
        ...
        >
          {TESTIMONIALS_DATA.map((testimonial, index) => (
            <SwiperSlide key={`testimonial-info-${index}`} styles>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
                <p style={{ height: '40px', width: '40px', display: 'flex', justifyContent: 'center', alignItems: 'center', borderRadius: '50%', backgroundColor: 'rgba(0,0,0,0.2)' }}>{index + 1}</p>
                <Box>
  ...
  testimonials: {
    ...
    '#testimonialsInfo': {
      ...
      '.swiper-slide': {
        ...
        paddingRight: ['0', null, null, null, null, '20px'],
